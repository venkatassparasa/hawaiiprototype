...
const GranularPermissions = () => {
    ...
    const handleEditRole = (id) => {
        ...
        const newDescription = prompt('Edit description:', role.description);
        
        if (newName && newDescription) {
            setUserRoles(userRoles.map(r => 
                r.id === id 
                    ? { ...r, name: newName, description: newDescription }
                    : r
            ));
        }
    };
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
            ...
            {activeTab === 'matrix' && (
                <div>
                    <div className="mb-6">
                        <h2 className="text-xl font-semibold text-slate-800 mb-4">Permission Matrix</h2>
                        <p className="text-sm text-slate-600 mb-4">Visual overview of permissions across all roles and modules.</p>
                    </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full min-w-full">
                                <thead className="bg-slate-50 border-b border-slate-200">
                                    <tr>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Role / Module</th>
                                        {modules.map((module) => (
                                            <th key={module.id} className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">
                                                <div className="flex flex-col items-center">
                                                    <span>{module.name}</span>
                                                    <span className="text-xs text-slate-400 normal">({module.id})</span>
                                                </div>
                                            </th>
                                        ))}
                                    </tr>
                                    ...
                                    {userRoles.filter(role => role.active).map((role) => (
                                        <tr key={role.id} className="hover:bg-slate-50">
                                            <td className="px-4 py-3 text-sm font-medium text-slate-900">
                                                <div>
                                                    <div>{role.name}</div>
                                                    <div className="text-xs text-slate-500">({role.templateId ? 'Template' : 'Custom'})</div>
                                                </div>
                                            </td>
                                            {modules.map((module) => {
